...
class InvoiceController extends Controller
{
    ...
    {
        $items = Item::Where('title', 'like', '%' . Input::get('term') . '%')->select(['id', 'title', 'sale_price'])->get();
        $items_array = array();
        $i = 0;
        foreach ($items as $item) {
            $items_array[$i]['value'] = $item->title;
            $items_array[$i]['id'] = $item->id;
            $items_array[$i]['sale_price'] = $item->sale_price;
            $i++;
        }
