...
class DashboardController extends Controller
{
    ...
    public function tiles()
    {
        ...
        $y = jDate::forge('now')->format('Y');
        $m = jDate::forge('now')->format('m');
        $start = jDateTime::toGregorian($y, $m, 1);
        if($m <= 6) {
            $finish = jDateTime::toGregorian($y, $m, 31);
        } else if($m == 12) {
            $finish = jDateTime::toGregorian($y, $m, 29);
        } else {
            $finish = jDateTime::toGregorian($y, $m, 30);
        }
        ...
        $num_of_dues = Invoice::due()->where('due_at', '<=', date('Y-m-d').' 00:00:00')->count();
