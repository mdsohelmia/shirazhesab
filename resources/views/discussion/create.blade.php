        ...
        <div class="col-md-12">
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item" aria-current="page"><a href="{{ route('discussion.create') }}">ایجاد بحث جدید</a></li>
                </ol>
            </nav>
                    ...
                    <div class="card card-default">
                        ...
                        <div class="card-body">
                            <form method="POST" action="{{ route('discussion.insert') }}" enctype="multipart/form-data">
                                ...
                                <div class="form-group row">
                                    <label for="text" class="col-md-4 col-form-label @lang('platform.input-pull')">متن</label>
                                    <div class="col-md-7">
                                ...
                                <div class="form-group row">
                                    <label for="important" class="col-md-4 col-form-label @lang('platform.input-pull')">مهم</label>
                                    <div class="col-md-7">
                                        <select id="important" name="important" class="form-control{{ $errors->has('type') ? ' is-invalid' : '' }}">
                                            <option value="yes"{{old('important') == 'yes' ? ' selected' : ''}}>بله</option>
                                            <option value="no"{{old('important') == 'no' ? ' selected' : ''}}>خیر</option>
                                        </select>
                                ...
                                <div class="form-group row">
                                    ...
                                    <div class="col-md-7">
                                        <select id="type" name="type" class="form-control{{ $errors->has('type') ? ' is-invalid' : '' }}">
                                            <option value="normal"{{old('type') == 'normal' ? ' selected' : ''}}>عادی</option>
                                            <option value="done"{{old('type') == 'done' ? ' selected' : ''}}>انجام شده</option>
                                            <option value="close"{{old('type') == 'close' ? ' selected' : ''}}>بسته شده</option>
                                            <option value="lock"{{old('type') == 'lock' ? ' selected' : ''}}>قفل شده</option>
                                        </select>
                                        @if ($errors->has('type'))
