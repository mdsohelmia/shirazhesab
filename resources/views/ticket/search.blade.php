            ...
            <form action="{{ route('ticket.search') }}" method="post">
                ...
                <div class="row">
                    <div class="col-md-7 mb-2">
                        <input type="text" name="keyword" value=" {{ old('keyword', $keyword) }}" class="form-control" id="keyword" placeholder="جستجو...">
                    </div>
                    <div class="col-md-3 mb-2">
                        <select name="status" class="form-control">
                            <option value="any"{{ old('status', $status) == 'any' ? ' selected' : ''}}>انتخاب وضعیت</option>
                            <option value="staff"{{ old('status', $status) == 'staff' ? ' selected' : ''}}>پاسخ پشتیبان</option>
                            <option value="user"{{ old('status', $status) == 'user' ? ' selected' : ''}}>پاسخ کاربر</option>
                            <option value="waiting"{{ old('status', $status) == 'waiting' ? ' selected' : ''}}>در انتظار بررسی</option>
                            <option value="close"{{ old('status', $status) == 'close' ? ' selected' : ''}}>بسته شده</option>
                            <option value="lock"{{ old('status', $status) == 'lock' ? ' selected' : ''}}>قفل شده</option>
                            <option value="done"{{ old('status', $status) == 'done' ? ' selected' : ''}}>انجام شده</option>
                        </select>
                    </div>
